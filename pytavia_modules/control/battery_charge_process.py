...
class battery_charge_process:
	...
	def execute(self, params):
		...
		try:
			...
			print ("===------------------------------------------===")
			print ("battery_live_state: " + str(battery_live_state))
			print ("battery_status: " + str(battery_status))
			print ("===------------------------------------------===")
			...
			if battery_live_state != battery_status:
				...
				if device_state_rec == None:
					...
					self.mgdDB.db_rt_device_state.update_one(
						{ "pid_code" : "BCS001" },
						{ "$set"	 : { "digital_state" : battery_status }}
					)
				# end if
			# end if
		except:
