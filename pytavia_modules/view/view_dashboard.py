...
class view_dashboard:
	...
	def get_charging_state(self, params):
		response = helper.response_msg(
			"DASHBOARD_SUCCESS",
			"DASHBOARD SUCCESS", {},
			"0000"
		...
		try:
			gpio_config_rec = self.mgdDB.db_rt_device_state.find_one(
				{ "pid_code" : "BCS001" }
			)
			gpio_status = "OFF"
			if gpio_config_rec != None:
				gpio_status  = gpio_config_rec["digital_state"]
				if gpio_status == 1:
					gpio_status = "OFF"
				else:	
					gpio_status = "ON"
	...
	def html(self, params):
		...
		try:
			...
			charging_state	 = self.get_charging_state( params )
			gpio_status		 = charging_state.get("data")["gpio_status"]
			...
			print ( "------------------------------------------------------" )
			print ( "gpio_status: " + str( gpio_status ) )
			print ( "------------------------------------------------------" )
			...
			template_html				= render_template(
				...
				force_battery_charging	= gpio_status,
				smart_battery_charging  = "OFF"
			)
			response.put( "data" , {
				"html" : template_html
			})
