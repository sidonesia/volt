...
function loadIcon(e) {
  if (e) {
    ...
    newinput1.id    = 'unsaved_' + counter + "_"
    newinput1.name  = 'thumbnails'
    newinput1.value =  JSON.stringify({ data:  croppedimage, counter : counter })
...
$('#form').validate({
  ...
  submitHandler: function(form) { 
    ...
    $.ajax ({
        ...
        data: params + "&" + $.param({ variance: JSON.stringify(variance) }, true) + "&" + $.param({ variance_permutate: variancePermutation }, true),
        success: function(response) {
          var fk_page_id = form.fk_page_id.value
          var success_url = '/admin/my-lynks/' + fk_page_id
